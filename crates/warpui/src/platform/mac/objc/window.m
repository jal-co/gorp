...
- (BOOL)performKeyEquivalent:(NSEvent *)event {
    ...
    if ([event type] == NSEventTypeKeyDown) {
        // Skip the key-equivalent priority path while the IME has marked text. Arrow keys
        // carry NSEventModifierFlagFunction, so AppKit delivers them here before keyDown:.
        // If we call keyDownImpl and Rust suppresses the keystroke (composing mode), we
        // return NO, and AppKit proceeds to call keyDown: — running interpretKeyEvents a
        // second time for the same event. See #9709.
